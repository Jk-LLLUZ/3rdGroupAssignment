...
<br>
The linear regression was calculated using the least squares method using its equation $\hat{y}=\hat{\beta_0}+\hat{\beta_1}x$ . Using this method, we can easily identify if the variables have positive or negative correlation between them using the slope intercept form $y=mx+b$. The researchers solved for the summations of $X_i$ and $Y_i$, the means of $x$ and $y$, the summations of squares $x_i$ and $y_i$, and the summation of $X_i$ times $Y_i$. After getting these values, the researchers solved for the $S_xy$ where it obtained a value of 488.54 and $S_xx$ that led to a value of 2,063.56. With these values, the estimates for regression coefficients can be easily calculated. With this, $\hat{\beta_1}= \frac{S_{xy}}{S_{xx}}$ and $\hat{\beta_0}=\overline{y}-\hat{\beta_1} \overline{x}$ can now be solved easily. Plugging in the values determines the fitted simple linear regression model, $\hat{y}=4.223932534x-6.738740311$. 
